...
在这里，我们直接照搬 Sidekiq Wiki 中 Getting Started 部分的代码简单展示下它是如何使用的，当我们执行 HardWorker.perform_async 方法时，Sidekiq 的 Worker 会将一个异步任务以 JSON 的形式将相关的信息加入 Redis 中并等待消费者对任务的拉取和处理。
